...
import { salePaymentSchema, type RequestSalePaymentDTO } from '../types/sale';
import {
...
function SaleSummaryPanel({ saleId }: { saleId: string }) {
  ...
  if (isLoading) return (
    <div className="bg-emerald-50 rounded-lg p-4 space-y-2 animate-pulse">
  ...
  return (
    <div className="bg-emerald-50 border border-emerald-100 rounded-lg p-4 space-y-1 text-sm">
      <p className="font-semibold text-emerald-800 mb-2">{summary.customerName} — {summary.saleNumber}</p>
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="bg-white rounded-md px-3 py-2 text-center">
          ...
          <p className="font-bold tabular-nums text-green-700">{formatCurrency(summary.receivedAmount)}</p>
        </div>
        <div className="bg-white rounded-md px-3 py-2 text-center">
...
function PendingSalePaymentsPanel() {
  ...
  if (pending.length === 0) return (
    <div className="text-center py-6 text-gray-400 text-sm">No pending receivables.</div>
  );
      ...
      <TableBody>
        {pending.map(s => (
          <TableRow key={s.publicId}>
            ...
            <TableCell className="text-right tabular-nums">{formatCurrency(s.totalAmount)}</TableCell>
            <TableCell className="text-right tabular-nums text-green-700">{formatCurrency(s.receivedAmount)}</TableCell>
            <TableCell className="text-right tabular-nums font-semibold text-red-700">{formatCurrency(s.pendingAmount)}</TableCell>
          </TableRow>
        ))}
      </TableBody>
    </Table>
...
export default function SalePayments() {
  ...
  const { data: paymentsPage, isLoading, isError, error, refetch } = useQuery({
    queryKey: ['salePayments', page],
    queryFn: () => getSalePayments(page, 15),
    enabled: activeTab === 'all',
  });

  const { data: salesPage } = useQuery({
    queryKey: ['sales', 0, {}],
    queryFn: () => getSales({ page: 0, size: 200 }),
  });
  ...
  const { register, handleSubmit, reset, control, formState: { errors } } = useForm<RequestSalePaymentDTO>({
  ...
  const pendingSales = salesPage?.content.filter(s => s.pendingAmount > 0) ?? [];
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <KpiCard
          ...
          value={formatCurrency(receivableTotal ?? 0)}
          icon={<TrendingUp className="h-5 w-5" />}
          trend={{ value: 0, label: 'Pending collection from customers' }}
      ...
      <div className="flex items-center space-x-1 bg-gray-100/50 p-1 rounded-lg w-fit">
        {(['all', 'pending'] as const).map(tab => (
          ...
          >
            ...
            {tab === 'all' ? 'All Payments' : 'Pending Receivables'}
          </button>
        ))}
      </div>
            ...
            <TableBody>
              {payments.map(p => (
                <TableRow key={p.publicId}>
                  ...
                  <TableCell className="font-mono text-xs text-gray-500">{p.saleNumber || '—'}</TableCell>
      ...
      <Modal isOpen={isModalOpen} onClose={handleCloseModal} title="Record Sale Payment" className="max-w-xl">
            ...
            >
              ...
              {pendingSales.map(s => (
                <option key={s.publicId} value={s.publicId}>
                  {s.saleNumber} — {s.customerName} (Pending: {formatCurrency(s.pendingAmount)})
                </option>
              ))}
            </select>
